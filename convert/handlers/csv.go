...
func init() {
	bld := types.NewBuilder(func(name named.Name, bundle *types.Bundle) (named.Named, error) {
		c, err := NewCsvConverter(name.ShortName().Name(), bundle)
		if err != nil {
			return nil, err
		}
	...
	for _, suffix := range []string{"csv"} {
		types.Register(types.NewComponent(named.Name(suffix+"."+types.EnginePandoc), bld))
		types.Register(types.NewComponent(named.Name(suffix+"."+types.EngineMarkItDown), bld))
	}
}
...
func NewCsvConverter(engine string, bundle *types.Bundle) (types.Converter, error) {
...
func (c *CsvConverter) Do(_ context.Context, doc *types.Document, insId string, _ map[string]any) ([]byte, error) {
	if mimeType, ok := mimetype.Valid(doc.Content, []string{"text/csv"}); !ok {
	...
	if strings.EqualFold(c.engine, types.EngineMarkItDown) {
		...
		if !ok {
			return nil, fmt.Errorf("pandoc instance: %s not found", insId)
		}
