...
func init() {
	bld := types.NewBuilder(func(name named.Name, bundle *types.Bundle) (named.Named, error) {
		c, err := NewOutlookConverter(name.ShortName().Name(), bundle)
		if err != nil {
			return nil, err
		}
		return types.NewWrapper(c), nil
	})
	for _, suffix := range []string{"msg"} {
...
type OutlookConverter struct {
	engine string
	logger log.Logger
	bundle *types.Bundle
}

func NewOutlookConverter(engine string, bundle *types.Bundle) (*OutlookConverter, error) {
...
func (c *OutlookConverter) Do(ctx context.Context, doc *types.Document, insId string, opts map[string]any) ([]byte, error) {
	if mimeType, ok := mimetype.Valid(doc.Content, []string{"application/vnd.ms-outlook"}); !ok {
