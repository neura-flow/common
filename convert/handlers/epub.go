...
func init() {
	bld := types.NewBuilder(func(name named.Name, bundle *types.Bundle) (named.Named, error) {
		c, err := NewEPubConverter(name.ShortName().Name(), bundle)
		if err != nil {
			return nil, err
		}
		return types.NewWrapper(c), nil
	})
	for _, suffix := range []string{"epub"} {
...
func NewEPubConverter(engine string, bundle *types.Bundle) (*EPubConverter, error) {
...
func (c *EPubConverter) Do(_ context.Context, doc *types.Document, insId string, _ map[string]any) ([]byte, error) {
	if mimeType, ok := mimetype.Valid(doc.Content, []string{"application/epub+zip"}); !ok {
